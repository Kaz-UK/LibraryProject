...
SELECT book.title, book.author, copies.loan_status FROM copies JOIN book ON copies.accession_id = book.accession_id
...
SELECT book.title, book.author, copies.loan_status FROM copies JOIN book ON copies.accession_id = book.accession_id WHERE book.title = '____';

-- As a user I want to borrow a book from the library so that I can read it.
INSERT INTO loans VALUES (null, date_issued, date_due, null, user_id, copy_id);
UPDATE copies  SET loan_status = 'On_loan' WHERE copy_id = __;

-- As a user I want to know when my book is due back to the library so that I can avoid a fine.
SELECT date_due FROM loans WHERE copy_id = __;

-- As a user I want to reserve a book that is currently on loan so that I can get it when it becomes available.
INSERT into reservations 
SELECT null, 'yyyy-mm-dd',3, loans.copy_id FROM copies JOIN book ON copies.accession_id = book.accession_id
join loans on loans.copy_id = copies.copy_id
...
SELECT book.title, book.author, copies.location FROM book JOIN copies on book.accession_id = copies.accession_id 
