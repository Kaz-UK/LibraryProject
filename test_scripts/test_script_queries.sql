...
SELECT COUNT(*) FROM copies WHERE accession_id = 1;

-- As a librarian I want to see user IDs of users who have books due on 2023-03-28.
SELECT user_id FROM loans WHERE date_due = '2023-03-28';
...
SELECT book.title, book.author, copies.loan_status FROM copies JOIN book ON copies.accession_id = book.accession_id WHERE book.title = 'Romeo and Juliet';

-- As a user I want to borrow a book from the library so that I can read it.
INSERT INTO loans VALUES (null, '2023-03-26', '2023-04-01', null, 1, 3);
UPDATE copies  SET loan_status = 'On loan' WHERE copy_id = 3;

-- As a user I want to know when my book is due back to the library so that I can avoid a fine.
SELECT date_due FROM loans WHERE copy_id = 2;

-- As a user I want to reserve a book that is currently on loan so that I can get it when it becomes available.
INSERT into reservations 
SELECT null,'2023-03-25',3, loans.copy_id FROM copies JOIN book ON copies.accession_id = book.accession_id
